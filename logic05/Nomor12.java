...
public class Nomor12 {
	...
	public static void main(String[] args) {
		...
		for (int i = 1; i < stringArray.length; i++) {//membandingkan j dg j-1 sesuai Abjad dan ASCII
			for (int j = i; j > 0; j--) {
		...
		String[][] string2dArray = new String[stringArray.length][];// dibuat array 2 dimensi
		...
		for (int i = 0; i < string2dArray.length; i++) {
			if (string2dArray[i] == null) {// ketika buah sama maka akan lanjut ke buah berikutnya
				continue;
			}
			else {
	            int sum = Integer.parseInt(string2dArray[i][1]);//menampung data 
